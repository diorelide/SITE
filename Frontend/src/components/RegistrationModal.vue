<template>
  <v-dialog v-model="visible" max-width="400px">
    <v-card>
      <v-card-title>Регистрация туриста</v-card-title>
      <v-card-text>
      ...
      <v-card-actions>
        ...
        <v-btn color="primary" @click="handleRegister">Зарегистрироваться</v-btn>
      </v-card-actions>
    </v-card>
  </v-dialog>
</template>
...
const handleRegister = async () => {
  ...
  try {
    const res = await fetch('http://localhost:3010/api/register', {
      method: 'POST',
